...
custom_version() {
    ...
    download_link="https://raw.githubusercontent.com/zamibd/MPanel/main/install.sh"
...
uninstall() {
    ...
    systemctl disable mpanel
    rm /etc/systemd/system/mpanel.service -f
    systemctl daemon-reload
    systemctl reset-failed
    rm /etc/mpanel/ -rf
    rm /usr/local/mpanel/ -rf

    echo ""
    echo -e "Uninstalled Successfully, If you want to remove this script, then after exiting the script run ${green}rm /usr/local/mpanel -f${plain} to delete it."
    echo ""
...
reset_admin() {
    ...
    if [[ $? == 0 ]]; then
        /usr/local/mpanel/mpanel admin -reset
    fi
    before_show_menu
}
...
reset_setting() {
    ...
    if [[ $? == 0 ]]; then
        /usr/local/mpanel/mpanel setting -reset
    fi
    before_show_menu
}
...
view_setting() {
    /usr/local/mpanel/mpanel setting -show
    view_uri
    before_show_menu
}

view_uri() {
    info=$(/usr/local/mpanel/mpanel uri)
    if [[ $? != 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/mpanel -N --no-check-certificate https://github.com/zamibd/MPanel/raw/main/mpanel.sh
    if [[ $? != 0 ]]; then
        echo ""
        LOGE "Failed to download script, Please check whether the machine can connect Github"
        before_show_menu
    else
        chmod +x /usr/bin/mpanel
...
check_mpanel_status() {
    count=$(ps -ef | grep "mpanel" | grep -v "grep" | wc -l)
    if [[ count -ne 0 ]]; then
        return 0
    else
        return 1
...
show_mpanel_status() {
    check_mpanel_status
    if [[ $? == 0 ]]; then
        echo -e "mpanel state: ${green}Running${plain}"
    else
        echo -e "mpanel state: ${red}Not Running${plain}"
    fi
}
...
show_usage() {
    echo -e "MPanel Control Menu Usage"
    echo -e "------------------------------------------"
    echo -e "SUBCOMMANDS:" 
    echo -e "mpanel              - Admin Management Script"
    echo -e "mpanel start        - Start mpanel"
    echo -e "mpanel stop         - Stop mpanel"
    echo -e "mpanel restart      - Restart mpanel"
    echo -e "mpanel status       - Current Status of mpanel"
    echo -e "mpanel enable       - Enable Autostart on OS Startup"
    echo -e "mpanel disable      - Disable Autostart on OS Startup"
    echo -e "mpanel log          - Check mpanel Logs"
    echo -e "mpanel update       - Update"
    echo -e "mpanel install      - Install"
    echo -e "mpanel uninstall    - Uninstall"
    echo -e "mpanel help         - Control Menu Usage"
    echo -e "------------------------------------------"
}

show_menu() {
  echo -e "
  ${green}MPanel Admin Management Script ${plain}
————————————————————————————————
  ${green}0.${plain} Exit
————————————————————————————————
...
————————————————————————————————
  ${green}11.${plain} MPanel Start
  ${green}12.${plain} MPanel Stop
  ${green}13.${plain} MPanel Restart
  ${green}14.${plain} MPanel Check State
  ${green}15.${plain} MPanel Check Logs
  ${green}16.${plain} MPanel Enable Autostart
  ${green}17.${plain} MPanel Disable Autostart
————————————————————————————————
...
————————————————————————————————
 "
    ...
    10)
        ...
        ;;
    11)
        check_install && start mpanel
        ;;
    12)
        check_install && stop mpanel
        ;;
    13)
        check_install && restart mpanel
        ;;
    14)
        check_install && status mpanel
        ;;
    15)
        check_install && show_log mpanel
        ;;
    16)
        check_install && enable mpanel
        ;;
    17)
        check_install && disable mpanel
        ;;
    18)
        bbr_menu
        ;;
...
if [[ $# > 0 ]]; then
    case $1 in
    "start")
        check_install 0 && start mpanel 0
        ;;
    "stop")
        check_install 0 && stop mpanel 0
        ;;
    "restart")
        check_install 0 && restart mpanel 0
        ;;
    "status")
        check_install 0 && status 0
        ;;
    "enable")
        check_install 0 && enable mpanel 0
        ;;
    "disable")
        check_install 0 && disable mpanel 0
        ;;
    "log")
        check_install 0 && show_log mpanel 0
        ;;
    "update")
