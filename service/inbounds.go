...
func (s *InboundService) GetAll() (*[]map[string]interface{}, error) {
	...
	for _, inbound := range inbounds {
		...
		if s.hasUser(inbound.Type) &&
			...
			users := []string{}
			err = db.Raw("SELECT clients.name FROM clients WHERE ? = ANY(SELECT value::int FROM jsonb_array_elements_text(clients.inbounds))", inbound.Id).Scan(&users).Error
			if err != nil {
				return nil, err
			}
			inbData["users"] = users
...
func (s *InboundService) addUsers(db *gorm.DB, inboundJson []byte, inboundId uint, inboundType string) ([]byte, error) {
	...
	condition := fmt.Sprintf("%d = ANY(SELECT value::int FROM jsonb_array_elements_text(clients.inbounds))", inboundId)
