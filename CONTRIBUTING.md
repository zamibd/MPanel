...
Thank you for your interest in contributing to MPanel. This document explains how to set up a development environment, follow project conventions, and submit changes. Your contributions help make the **multi-inbound-per-user** approach and the rest of the project better for everyone.
...
1. Build and run with the provided script (builds backend and runs with debug + local DB):
   ...
   This runs `./build.sh` then `MPANEL_DB_FOLDER="db" MPANEL_DEBUG=true ./mpanel`.
...
2. **Replace web assets and build backend**:
   ...
   go build -ldflags "-w -s" -tags "with_quic,with_grpc,with_utls,with_acme,with_gvisor,with_tailscale" -o mpanel main.go
   ```
...
|----------------|--------------------------------|-----------|
| `MPANEL_DB_FOLDER`| Directory for SQLite DB files  | `db`      |
| `MPANEL_DEBUG`    | Enable debug mode              | `true`    |
| `MPANEL_LOG_LEVEL`| Log level                      | `debug`   |
| `MPANEL_BIN_FOLDER` | Directory for binaries       | `bin`     |
...
- The project does not yet have a formal test mpanelte (no `*_test.go` files in the repo).
...
2. **Manual testing**: Run with `./runMPanel.sh`, test the changed area (panel, API, subscription, etc.).
...
Community help is especially valuable in these areas. Check the [Issues](https://github.com/zamibd/MPanel/issues) for current tasks and ideas.

### High-Value Areas

- **Multi-inbound per user**: Core differentiator of MPanel; improvements to UX, docs, and robustness are welcome.
- **API (v1 and v2)**: Completeness, consistency, and documentation (see [API Documentation](https://github.com/zamibd/MPanel/wiki/API-Documentation)).
...
4. **When forking**  
   If you fork MPanel, `CONTRIBUTING.md` is already in the repo. Update the clone URLs and repo names in this file if you want your fork’s contribution instructions to point to your own repository.
...
Thank you for helping MPanel grow. Your contributions make it possible for more users to adopt MPanel in production and benefit from its multi-inbound-per-user design.
