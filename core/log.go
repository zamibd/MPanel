...
func (p PlatformWriter) WriteMessage(level log.Level, message string) {
	switch level {
	case log.LevelInfo:
		mpanelLog.Info(message)
	case log.LevelWarn:
		mpanelLog.Warning(message)
	case log.LevelPanic:
	case log.LevelFatal:
	case log.LevelError:
		mpanelLog.Error(message)
	default:
		mpanelLog.Debug(message)
	}
}
...
func (l *observableLogger) Log(ctx context.Context, level log.Level, args []any) {
	...
	case log.LevelWarn:
		mpanelLog.Warning(l.tag, msg)
	case log.LevelPanic:
	case log.LevelFatal:
	case log.LevelError:
		mpanelLog.Error(l.tag, msg)
	default:
		mpanelLog.Debug(l.tag, msg)
	}
